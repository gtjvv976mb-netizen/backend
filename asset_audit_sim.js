...
let a=await audit();
chk(!!a.units && !!a.units.u1, "the wallet now has an asset record");
...
await save({onboarded:true, units:{u1:{species:"dragonos",kind:"legendary",level:20}}, mounts:["horse"], eggs:[{kind:"legendary",started:5000}]});
SRV._ageAssetEggs(W, 13*3600*1000);   // a legendary egg is 12h; the server times it, not the save
...
await save({onboarded:true, units:{u1:{species:"dragonos",kind:"legendary",level:20}, u2:{species:"galador",kind:"legendary",level:1}, u7:{species:"tyrannos",kind:"legendary",level:50}}, mounts:["horse"], eggs:[]});
a=await audit();
chk(a.units.u7 && a.units.u7.origin==="unverified", `the conjured legendary is 'unverified' (${a.units.u7&&a.units.u7.origin})`);
...
await save({onboarded:true, units:{u1:{species:"dragonos",kind:"legendary",level:20}}, mounts:["horse"], eggs:[]});
a=await audit();
chk(!!a.units.u7, "deleting the forged chikimon does NOT erase its record");
...
console.log("— the record survives a server restart —");
{
  ...
  await save({ eggs: [], units: { u1:{species:"pyrolisk",kind:"legendary",level:40}, u2:{species:"leafcub",kind:"normal",level:5},
                                  u7:{species:"voidwyrm",kind:"legendary",level:50} }, mounts: ["griffin","direhog"] });
  const relaunder = await audit();
  chk(relaunder.units.u7.origin === "unverified",
     `a save right after the restart does NOT re-stamp it legacy (${relaunder.units.u7.origin})`);
}
...
console.log("— overflowing the ledger evicts the clean, never the flagged —");
{
  ...
  await save({ eggs: [], units: { u1:{species:"pyrolisk",kind:"legendary",level:40}, u2:{species:"leafcub",kind:"normal",level:5},
                                  u7:{species:"voidwyrm",kind:"legendary",level:50}, u8:{species:"aurorox",kind:"legendary",level:50} },
  ...
  chk(mine.units.u7 && mine.units.u7.origin === "unverified", `and kept its verdict (${mine.units?.u7?.origin})`);
  chk(mine.unverified >= keep, `its flag count did not drop (${mine.unverified} >= ${keep})`);
  chk(mine.units.u8 && mine.units.u8.origin === "unverified", `a 4th conjured unit is still caught post-eviction (${mine.units?.u8?.origin})`);
}
