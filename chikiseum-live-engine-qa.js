...
const files = ['chikiseum-live-engine.js', 'chikiseum-live-engine.test.js', 'chikiseum-live-parity.test.js',
  ...
  'chikiseum-profiles.json', 'chikiseum_reference_arena_v2.json', 'CHIKISEUM-LIVE-ENGINE.md',
...
if (!/tests 34\b/.test(stdout) || !/pass 34\b/.test(stdout) || !/fail 0\b/.test(stdout)) throw new Error('Incomplete engine/navigation test gates');
...
const receipt = { schema: 'chikiseum.production-realtime-engine-qa/v1', status: 'pass', tests: 34, failures: 0,
...
console.log(JSON.stringify({ receipt: prefix + '.json', sha256: sha(json), log: prefix + '.log', log_sha256: sha(stdout), tests: 34, failures: 0 }));
